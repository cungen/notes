...
可以建立多个虚拟workspace（桌面），在一个workspace中，分配多个应用并记录布局方式
...
- 窗口平铺 LR、TB
	...
	- 大小调整：`Alt + Shift + [-+]`
- Float窗口
	- toggle float窗口`Alt + Shift + ;`松开后按 `f` 
- Workspace
	- 分配窗口：`Alt + Shift + {workspace id}`
	- 切换Workspace：`Alt + {workspace id}`
	- 切换Screen：`Alt + Shift + tab`
## 体验总结
...
[[on-window-detected]]
# if.app-id = '(com.tencent.xinWeChat|com.tencent.WeWorkMac)'
if.window-title-regex-substring = '(WeChat|WeCom|豆包|Claude|Preview|智谱清言)'
run = 'layout floating'
